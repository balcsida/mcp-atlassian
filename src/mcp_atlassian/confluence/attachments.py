...
class AttachmentsMixin(ConfluenceClient, AttachmentsOperationsProto):
    ...
    def _send_attachment_request(
        ...
        # Confluence Server/DC requires "no-check" (with hyphen) to bypass
        # XSRF validation on multipart uploads.
        ...
        # Use POST to create a new attachment. PUT on the list endpoint is not
        # supported by Confluence Server/DC and returns 404 or 405.
        response = self.confluence._session.post(
            url, headers=headers, files=files, data=data
        )

        # On Server/DC, uploading a file that already exists returns HTTP 400
        # with "same file name" in the response. In that case locate the existing
        # attachment by filename and POST to its /data endpoint to create a new
        # version instead.
        if response.status_code == 400 and "same file name" in response.text:
            ...
            if att_results:
                ...
                if hasattr(file_obj, "seek"):
                    file_obj.seek(0)
                response = self.confluence._session.post(
                    update_url, headers=headers, files=files, data=data
                )
