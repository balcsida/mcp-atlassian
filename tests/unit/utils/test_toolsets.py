...
class TestGetEnabledToolsets:
    ...
    def test_all_keyword(self, monkeypatch):
        """Test 'all' keyword returns all 27 toolset names."""
    ...
    def test_all_toolsets_count(self):
        """Verify ALL_TOOLSETS has exactly 27 entries."""
        assert len(ALL_TOOLSETS) == 27
    ...
    def test_all_toolsets_contains_jira_and_confluence(self):
        ...
        assert len(jira_toolsets) == 18
        assert len(confluence_toolsets) == 9
...
class TestToolsetTagCompleteness:
    ...
    def test_jira_tool_count(self, jira_tools):
        ...
        assert len(jira_tools) == 67, f"Expected 67 Jira tools, got {len(jira_tools)}"

    def test_confluence_tool_count(self, confluence_tools):
        """Verify expected number of Confluence tools."""
        assert len(confluence_tools) == 39, (
            f"Expected 39 Confluence tools, got {len(confluence_tools)}"
        )
